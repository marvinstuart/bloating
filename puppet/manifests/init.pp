...
node 'default' {
  ...
  file { 'bloating-vhost':
    path => '/etc/apache2/sites-enabled/000-bloating',
    ensure => 'link',
    target => '/home/pi/bloating/bloating.vhost',
    require => Package['libapache2-mod-wsgi'],
  }

  # Restart Apache.
  exec { 'apache2ctl restart':
    require => File['bloating-vhost'],
  }

  # Set up cron jobs.
  cron { 'measure-ds18b20':
    command => '/home/pi/bloating/bloating/measure-ds18b20.py >/dev/null 2>&1',
    user => root,
    minute => '*/5',
  }
}
